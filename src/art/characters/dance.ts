...
export function kickEnvelope(danceT: number) {
  if (danceT < 1.35) {
    if (danceT < 0.22) return 0;
    if (danceT < 0.36) return (danceT - 0.22) / 0.14;
    if (danceT < 1.12) return 1;
    return Math.max(0, 1 - (danceT - 1.12) / 0.2);
  }
  const u = ((danceT - 1.35) % 1.15) / 1.15;
  if (u < 0.28) return 0;
...
export function kickSide(danceT: number): 1 | -1 {
  if (danceT < 1.35) return 1;
  return Math.floor((danceT - 1.35) / 1.15) % 2 === 0 ? -1 : 1;
}
